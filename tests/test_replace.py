...
class TestBloomReplace(unittest.TestCase):
    ...
    def _init_dnas(self):
        ...
        while len(self.dnas) < self.count:
            dna = [random.choice([0, 1, 2, 3]) for _ in range(self.dna_length)]
    ...
    def test_wrong_matrix(self):
        total = int(math.pow(4, self.dna_length))
        error_count = 0
        for number in range(total):
            ...
            while number > 0:
                ...
                number //= 4
            dna = ["A" for _ in range(self.dna_length - len(dna))] + dna
